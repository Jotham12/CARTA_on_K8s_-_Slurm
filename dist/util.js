...
function getUserFolders(rootFolderTemplate, baseFolderTemplate, username) {
    ...
    const userScopedRootFolder = getUserScopedFolder(rootFolderTemplate, username);
    const userScopedBaseFolder = getUserScopedFolder(baseFolderTemplate, username);
    // Clamp the visible top-level folder to the user's own directory when the
    // configured root is shared, so the browser opens at the user folder.
    const topLevelFolder = (_a = userScopedRootFolder !== null && userScopedRootFolder !== void 0 ? userScopedRootFolder : userScopedBaseFolder) !== null && _a !== void 0 ? _a : rootFolder;
    return {
        topLevelFolder,
        baseFolder
    };
